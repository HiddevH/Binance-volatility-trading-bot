...
# Load creds modules
from helpers.handle_creds import load_correct_creds, test_api_key
...
def get_price(add_to_historical=True):
    ...
    for coin in prices:
        ...
        if CUSTOM_LIST:
            ...
            if PAIR_WITH in coin["symbol"] and all(
                item not in coin["symbol"] for item in FIATS
            ):
                initial_price[coin["symbol"]] = {
                    "price": coin["price"],
...
def wait_for_price():
    ...
    if historical_prices[hsp_head]["BNB" + PAIR_WITH][
        "time"
        ...
        time.sleep(
            (
                ...
                - (
                    datetime.now()
                    - historical_prices[hsp_head]["BNB" + PAIR_WITH]["time"]
                )
            ).total_seconds()
        )

    print(
        f"Working...Session profit:{session_profit:.2f}% Est:${(QUANTITY * session_profit)/100:.2f}"
    )
    ...
    for coin in historical_prices[hsp_head]:
        ...
        if threshold_check > CHANGE_IN_PRICE:
            ...
            if coin not in volatility_cooloff:
                volatility_cooloff[coin] = datetime.now() - timedelta(
                    minutes=TIME_DIFFERENCE
                )

            # only include coin as volatile if it hasn't been picked up in the last TIME_DIFFERENCE minutes already
            if datetime.now() >= volatility_cooloff[coin] + timedelta(
                minutes=TIME_DIFFERENCE
            ):
                volatility_cooloff[coin] = datetime.now()

                if (
                    len(coins_bought) + len(volatile_coins) < MAX_COINS
                    or MAX_COINS == 0
                ):
    ...
    for excoin in externals:
        if (
            ...
            and excoin not in coins_bought
            and (len(coins_bought) + exnumber + len(volatile_coins)) < MAX_COINS
        ):
            volatile_coins[excoin] = 1
            exnumber += 1
            print(
                f"External signal received on {excoin}, calculating volume in {PAIR_WITH}"
            )
...
def buy():
    ...
    for coin in volume:
        ...
        if coin not in coins_bought:
            ...
            if TEST_MODE:
                orders[coin] = [
                    {"symbol": coin, "orderId": 0, "time": datetime.now().timestamp()}
                ]
...
def sell_coins():
    ...
    for coin in list(coins_bought):
        ...
        SL = (
            float(coins_bought[coin]["bought_at"])
            + (float(coins_bought[coin]["bought_at"]) * coins_bought[coin]["stop_loss"])
            / 100
        )
        ...
        if LastPrice > TP and USE_TRAILING_STOP_LOSS:
            ...
            coins_bought[coin]["stop_loss"] = (
                coins_bought[coin]["take_profit"] - TRAILING_STOP_LOSS
            )
            if DEBUG:
                print(
...
if __name__ == "__main__":
    ...
    else:  # use the old fashioned way
